...
> Update `YOUR_USERNAME/YOUR_REPO` above (and in the first code cell of the notebook) with the actual path of this repository once it is pushed to GitHub, so that the badge opens this exact notebook in Google Colab.
...
├── report.tex / report.pdf        # full project report (LaTeX)
...
**Option A — Google Colab (recommended, zero setup):** click the badge above.
...
See `report.pdf` (source in `report.tex`) for the full methodology, scalability analysis, and discussion of results.
